...
import { baseline, head, readCandidate, candidateHealth, executeHandCommand, recoverHandCandidate, SEMANTIC_STATE } from '../src/render/janko/semantic-hand';
...
async function main() {
  ...
  const result = action === 'recover' ? recoverHandCandidate(input, root, path) :
    executeHandCommand(action === 'change' ? { action, request: input } : { action: 'undo', base: input.base, revision: input.revision }, root, path);
  ...
  let publication: { state: 'pending' | 'ready'; generation?: string; generationMs?: number; reason?: string } = { state: 'pending', reason: 'dev studio not observed' };
  ...
  if (timeout > 0) {
    while (performance.now() - publishStart < timeout) {
      try {
        ...
        if (response.ok) {
          const status = await response.json() as { candidateRevision?: string; candidateError?: string; generation: string; generationMs?: number; stale: boolean };
          publication = status.candidateRevision === result.revision && !status.stale && !status.candidateError
            ? { state: 'ready', generation: status.generation, generationMs: status.generationMs }
  ...
  return { ...result, publication, timingsMs: { resolve: +resolveMs.toFixed(1), deriveLayoutLint: +deriveLayoutLintMs.toFixed(1), publicationWait: +(performance.now() - publishStart).toFixed(1), commandTotal: +(performance.now() - started).toFixed(1) } };
}

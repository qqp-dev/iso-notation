 ...
 *
 * Ten authorized LH → RH retargetings — the six descending-line eighths of
 * mm. 23/43 plus the four phrase-continuation notes of mm. 24/44 — applied as
 * a declarative score-layer table AFTER the validated written-duration
 * overlay (original MIDI-track keys) and BEFORE hand-crossing computation.
 * Source cross-staff movement never forces a hand change by itself; only
 * this table retargets, with exact guards, and whole-piece hand fidelity
 * remains uncertified (notably the mm. 61–62 cross-voice unison).
 */
...
/** The ten authorized targets: id → [pitchClass, octave, startTick, duration]. */
...
test('Table v2 carries exactly the ten authorized corrections', () => {
  assert.equal(BRAHMS_HAND_CORRECTIONS_VERSION, 2, 'amendment version');
  assert.equal(BRAHMS_HAND_CORRECTIONS.length, 10, 'ten records, no more');
  const ids = BRAHMS_HAND_CORRECTIONS.map((c) => c.expectedId);
  assert.deepEqual([...ids].sort(), [...TEN.keys()].sort(), 'exactly the ten targets');
  for (const c of BRAHMS_HAND_CORRECTIONS) {
    const [pc, oct, tick, dur] = TEN.get(c.expectedId)!;
...
test('Ten and only ten LH→RH changes; every other field of all 964 preserved', () => {
  ...
  for (let i = 0; i < 964; i++) {
    ...
    if (a.hand !== b.hand) {
      changed.push(a.id);
      assert.equal(a.hand, 'LH', `${a.id}: was LH`);
      assert.equal(b.hand, 'RH', `${a.id}: now RH`);
    }
  }
  assert.deepEqual(changed.sort(), [...TEN.keys()].sort(), 'exactly the ten retargets');
});
...
test('Literal m.23/m.43: full RH run over LH bass; the four bridges dissolve', () => {
  ...
  assert.equal(bridges, 35, 'four bridges gone (was 39)');

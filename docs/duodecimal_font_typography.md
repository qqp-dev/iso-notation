...
3. **Handedness Indicator**: **Intuitive Up/Down Chevrons (`∧`, `∨`)**.
   - The chevron always points back toward Middle C, reading instantly as *"this hand is out of its home register"*.
   - **RH below Middle C**: an upward chevron (`∧`) sits above the notehead — *"Right Hand playing down here!"*
   - **LH above Middle C**: a downward chevron (`∨`) sits below the notehead — *"Left Hand playing up here!"*
   - Authoritative $4.2 \times 2.8\text{pt}$ open chevrons with a $1.20\text{pt}$ round-capped/round-joined stroke in the note's duration hue, held $2.2\text{pt}$ clear of the circular knockout so they read instantly at a glance.
4. **Horizontal System Architecture & Classical Vertical Accolade**:
   - Time flows left → right while pitch rises vertically from o1 (bottom) to o5 (top); landscape pages stack 4 measures per system and 3 systems per page (12 measures/page).
   - A slender copperplate Urtext curly brace ($7.0\text{pt}$ reach, $0.85\text{pt}$ swell) clasps the full 4-octave staff vertically on the left margin of every system, its sharp central cusp pointing directly into the bold Middle C spine. The staff lines emerge openly from the left with no staff-bounding opening barline.
   - Noble concentric halo ring framing the opening sounds at tick 0 in Measure 1.
   - Clean Urtext measure numerals above the FIRST measure of each system only (m. 1, 5, 9, …); zero per-measure counters, zero confusing margin numeral stacks and zero column-top octave badges.
5. **Horizontal Hold Lines & Sensible Continuity**:
   - Duration trails extend horizontally to the right in the note's duration hue.
   - Every tail begins flush against the effective circular boundary of its onset (the 5.80pt halo ring for the tick-0 opening sounds, the 4.80pt knockout otherwise) and stops clear of any following note on the same pitch.
   - On staff lines they continuously color the line (1.35pt on the bold Middle C spine, 0.65pt on the octave lines, `stroke-linecap="butt"`); in open space they are $0.80\text{pt}$ round-capped trails nudged 0.4pt clear of the circle.

---

## 7. Definitive Score Renders

*All crops below were historically generated from the horizontal landscape engraving (commit `f33d9e0`) and are preserved as committed benchmark assets.*

### Measures 1–2 (Classical Vertical Accolade, Opening Position of Honor):
![Definitive mm. 1-2](img/definitive_m1_m2.png)

### Measure 4 (RH-in-Bass Sequence `9 ∧`, `7 ∧`, `6 ∧`, `9 ∧`, `0 ∧` with Upward Chevrons):
![Definitive m. 4](img/definitive_m4.png)

### Measure 6 (Continuous Royal Blue Hold Line on the Middle C Spine):
![Definitive m. 6](img/definitive_m6.png)

### Measure 30 (LH-in-Treble Arpeggio `2 ∨`, `4 ∨`, `6 ∨`, `b ∨` with Downward Chevrons):
![Definitive m. 30](img/definitive_m30.png)

### Full Page 1 of the 3-Page Landscape Spread (Measures 1–12 across 3 Systems):
![Horizontal landscape page 1](img/horizontal_portrait_page1.png)

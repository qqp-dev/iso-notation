...
3. **Handedness Indicator**: **Intuitive Up/Down Chevrons (`∧`, `∨`)**.
   ...
   - Rendered as an open round-capped vector stroke ($0.80\text{pt}$) in the note's duration hue, so it never obscures the digit or neighboring staff lines.
4. **Horizontal System Architecture & Classical Vertical Accolade**:
   - Time flows left → right while pitch rises vertically from o1 (bottom) to o5 (top); portrait pages stack 4 measures per system and 4 systems per page (16 measures/page).
   - A copperplate Urtext curly brace clasps the full 4-octave staff vertically on the left margin of every system, its central cusp pointing directly into the bold Middle C spine.
   - Noble concentric halo ring framing the opening sounds at tick 0 in Measure 1.
   - Clean Urtext measure numerals above each measure's opening vertical barline; zero confusing margin numeral stacks and zero column-top octave badges.
5. **Horizontal Hold Lines & Sensible Continuity**:
   - Duration trails extend horizontally to the right in the note's duration hue.
   - On staff lines they continuously color the line (1.35pt on the bold Middle C spine, 0.65pt on the octave lines, `stroke-linecap="butt"`); in open space they are $0.8\text{pt}$ round-capped trails.

---

## 7. Definitive Score Renders

*All crops below are generated by `npx tsx scripts/render-docs-crops.ts` from the horizontal portrait engraving.*
...
### Full Page 1 of the 2-Page Zero-Turn Spread (Measures 1–16 across 4 Systems):
![Horizontal portrait page 1](img/horizontal_portrait_page1.png)
